...
public class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        Text = "Autofocus Manual Control v6.9";
    ...
    private void BuildControlPanel(Control parent)
    {
        ...
        btnHalt.Click += (s, e) => { _serial.SendCommand(":FQ#", false); };
    ...
    private void BuildDevicePanel(Control parent)
    {
        ...
        // 设备连接
        var gbConn = new GroupBox { Text = "设备连接", Location = new Point(10, y), Size = new Size(248, 127) };
        AddLabel(gbConn, "串口号:", 12, 25, 50, 20);
        cbPort = new ComboBox { Location = new Point(65, 23), Size = new Size(100, 22), DropDownStyle = ComboBoxStyle.DropDownList };
        gbConn.Controls.Add(cbPort);
        btnScan = new Button { Text = "扫描", Location = new Point(170, 22), Size = new Size(60, 24) };
        btnScan.Click += (s, e) => ScanPorts();
        gbConn.Controls.Add(btnScan);

        AddLabel(gbConn, "波特率:", 12, 55, 50, 20);
        cbBaudRate = new ComboBox { Location = new Point(65, 53), Size = new Size(100, 22), DropDownStyle = ComboBoxStyle.DropDownList };
        cbBaudRate.Items.AddRange(new object[] { "115200", "57600", "38400", "19200", "9600" });
        cbBaudRate.SelectedIndex = 0;
        gbConn.Controls.Add(cbBaudRate);

        btnConnect = StyledButton("连接", 14, 88, 82, 28, Color.FromArgb(0, 120, 212), Color.White);
        btnConnect.Click += (s, e) => ConnectDevice();
        gbConn.Controls.Add(btnConnect);

        btnDisconnect = StyledButton("断开", 102, 88, 82, 28, Color.White, Color.Black);
    ...
    private void BuildLogTab(TabPage tab)
    {
        ...
        btnCopyLog = new Button { Text = "一键复制", Location = new Point(182, 6), Size = new Size(75, 26) };
    ...
    {
        if (_config.AutoConnect && !string.IsNullOrEmpty(_config.PortName))
        {
    ...
    private void MoveRelative(int steps)
    {
        ...
        // Vestaline: set target (8-char hex, firmware uses hex2u32)
        _serial.SendCommand(":SN" + target.ToString("X8") + "#", false);
        _serial.SendCommand(":FG#", false);
        _currentPosition = target;
        lblPosition.Text = _currentPosition.ToString();
        AddLog("前往: " + target, LogType.Info);
    }
    ...
    private void SaveFocalPlane()
    {
        ...
        AddLog("焦平面已保存: " + _currentPosition, LogType.Info);
    }
    ...
    private void SaveSettings()
    {
        ...
        // 如果连接参数变了，提示重连
        if (_serial.IsConnected && (_config.PortName != oldPort || _config.BaudRate != oldBaud))
    ...
    private void AddLog(string message, LogType type)
    {
        ...
        string prefix = type == LogType.Send ? ">>>" : type == LogType.Receive ? "<<<" :
                       type == LogType.Error ? "!!!" : "---";
        Color color = type == LogType.Send ? Color.Blue :
                     type == LogType.Receive ? Color.Green :
                     type == LogType.Error ? Color.Red : Color.Gray;
    ...
    private void CopyLog()
    {
        ...
        t.Tick += (s, e) => { btnCopyLog.Text = "一键复制"; t.Stop(); t.Dispose(); };
        t.Start();
    }
    ...
    private void PollTimer_Tick(object sender, EventArgs e)
    {
        ...
        if (_pollPhase % 2 == 1)
        {
            // :GP# returns 4-char hex position, e.g. "0000"
            string resp = _serial.SendCommand(":GP#");
        ...
        {
            // :GT# returns 4-char hex temp*2, e.g. "0081"
            string resp = _serial.SendCommand(":GT#");
            if (resp != null && resp.Length >= 2)
            {
                int raw;
            ...
            // :GI# returns "00" (stopped) or "01" (moving)
            string sr = _serial.SendCommand(":GI#");
